...
- Run `stow-all.ps1` from an elevated PowerShell. A symlink created by a non-elevated process - the Developer Mode path - is an untrusted reparse point, and Windows will not traverse one for a file open whose token is a network logon, which is what OpenSSH public-key auth produces. Inside an `ssh` session every stowed dotfile then fails with `error 448: the path cannot be traversed because it contains an untrusted mount point` while resolving fine locally, and git degrades to `fatal: unknown error occurred while reading the configuration files` because `~/.gitconfig` is unreadable. Nothing in `Get-Item` or `fsutil reparsepoint query` tells the two apart - tag, flags and substitute name are byte-identical - only the link's owner differs (`BUILTIN\Administrators` versus the user SID). A matching link target is therefore not proof the link works: `stow-all.ps1` opens every link it would otherwise report as `ok`, repairs untrusted ones when elevated, and warns when not. Repo-side symlinks that git itself checks out (`common/pymol/.pymolrc*`) hit the same rule; recreate those preserving their **relative** target so git stays clean.
