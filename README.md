...
Codex applies the most restrictive matching rule across active files. The portable file contains only `prompt` guardrails for directly destructive commands, destructive Git history/worktree operations, and privilege escalation; it grants no cross-host `allow`. Routine workspace-local execution, network and transfer clients, package managers, Git/GitHub operations, and other host-specific exceptions are left to Codex's sandbox and host-local approvals, avoiding a second portable prompt layer. One host's project-specific approvals therefore cannot silently weaken another host. Rerun `./stow-all.sh <host>` after pulling portable policy changes.
