...
if [[ ! $type =~ image/ ]]; then
  ...
  # Sometimes bat is installed as batcat, but the version is not compatible.
...
if { [[ -n $KITTY_WINDOW_ID ]] || [[ -n $GHOSTTY_RESOURCES_DIR ]]; } &&
  ...
  kitten icat --clear --transfer-mode=stream --unicode-placeholder --stdin=no --place="$dim@0x0" "$file" | sed '$d' | sed $'$s/$/\e[m/'
