...
download() {
    if (($# == 0)); then
        echo "Usage: download <file_or_dir> [more_files_or_dirs...]" >&2
        return 2
    fi

    if ! command -v kitten >/dev/null 2>&1; then
        echo "download: 'kitten' not found. Tip: login using 'kitten ssh user@host' (recommended) or install kitty/kitten on this remote." >&2
        return 127
    fi
